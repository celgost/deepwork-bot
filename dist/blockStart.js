...
function cronForStart(start) {
    const { hour, minute } = parseHHMM(start);
    return `${minute} ${hour} * * *`;
}
export async function sendBlockStartMessages(client, guildId, block, options) {
    ...
    if (textChannel && textChannel.type === ChannelType.GuildText) {
        await textChannel.send(`🧠 Block ${block} started\n\n` +
    ...
    if (dw50Channel && dw50Channel.isTextBased()) {
        await dw50Channel.send(`<@&${ROLES.deepWork50}>\n` +
    ...
    if (dw100Channel && dw100Channel.isTextBased()) {
        await dw100Channel.send(`<@&${ROLES.deepWork100}>\n` +
            "What is your todolist for this session?\n" +
            "Write it here concisely.");
    }
    if (!options?.force) {
...
export function scheduleBlockStarts(client, guildId) {
