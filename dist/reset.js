...
import { markExecuted, resetPersistedState, setDailyMessageId, shouldRun, } from "./state.js";
const PIN_MARKER = "**Deep Work Today**";
async function unpinExistingDailyMessage(channel) {
    const pins = await channel.messages.fetchPins();
    const toUnpin = pins.items.filter((pin) => pin.message.author.id === channel.client.user?.id &&
...
async function clearDeepWorkRoles(client, guildId) {
...
export async function resetDaily(client, guildId) {
    if (!shouldRun("daily-reset"))
        return;
    const guild = await client.guilds.fetch(guildId);
