...
import { buildAttendeeMap, updateDailyMessage } from "./attendees.js";
...
async function isAdmin(message: Message): Promise<boolean> {
  if (ADMIN_IDS.includes(message.author.id)) return true;
  if (!message.member) return false;
  if (!ID_REGEX.test(ADMIN_ROLE_ID)) return false;
...
async function fetchDailyMessageForGuild(
...
async function postDailyForGuild(
  client: Message["client"],
  guildId: string
): Promise<void> {
...
async function syncFromReactionsForGuild(
  client: Message["client"],
  guildId: string
): Promise<void> {
...
function formatState(): string {
  const blockSummary = (block: BlockKey): string => {
...
export async function handleTestCommands(message: Message): Promise<boolean> {
  if (!message.guild || message.author.bot) return false;
  const content = message.content.trim();
  if (!content.startsWith("!")) return false;
  const [command, ...args] = content.slice(1).split(/\s+/);
  if (!command.startsWith("test_")) return false;
...
export function buildTestCommands(): Array<SlashCommandBuilder | SlashCommandOptionsOnlyBuilder> {
  return [
...
export async function registerTestCommands(clientId: string, guildId: string): Promise<void> {
...
export async function handleTestInteraction(
  ...
  if (!interaction.commandName.startsWith("test_")) return false;
  ...
  try {
    ...
    const block = interaction.options.getString("block")?.toUpperCase() as BlockKey | undefined;
    ...
    if (interaction.commandName === "test_run_timers") {
      ...
      for (const m of [50, 60, 100] as const) {
